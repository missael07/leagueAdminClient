    ...
    <v-container grid-list-xs>
        <v-card rounded="lg">
                ...
                <v-form>
                    <v-row>
                        <v-col :cols="$vuetify.display.mdAndDown ? 12 : 6" align="center">
                            <v-avatar v-if="roster.imgUrl" size="200">
                            ...
                            <v-row>
                                <v-col :cols="$vuetify.display.mdAndDown ? 6 : 6">
                                    <v-text-field v-model="roster.firstName" :label="Labels.rosterLabels.nameText"
                                        :error-messages="getErrors(errors, 'firstName')" readonly />
                                </v-col>
                                <v-col :cols="$vuetify.display.mdAndDown ? 6 : 6">
                                    <v-text-field v-model="roster.lastName" :label="Labels.rosterLabels.lastNameText"
                                        :error-messages="getErrors(errors, 'lastName')" readonly />
                                </v-col>
            ...
            <v-card-actions class="d-flex justify-end mr-4">
                <v-btn text="Cancelar" variant="plain" :to="`/admin/rosters/team/${teamId}`" />
...
import useAdminRoster from './composables/useAdminRoster';


const { roster, editRoster } = useEditRoster();
const { getRoster } = useAdminRoster();
